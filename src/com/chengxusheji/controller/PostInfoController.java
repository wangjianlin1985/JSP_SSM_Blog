...
public class PostInfoController extends BaseController {
	...
	public void add(@Validated PostInfo postInfo, BindingResult br,
		...
		try {
			postInfo.setPostPhoto(this.handlePhotoUpload(request, "postPhotoFile"));
	...
	/*客户端ajax方式 用户 提交添加博文信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(@Validated PostInfo postInfo, BindingResult br,
	...
	public String frontshow(@PathVariable Integer postInfoId,Model model,HttpServletRequest request,HttpSession session) throws Exception {
        ...
        if(user_name != null) {
        	...
        	zanState = zanInfoService.queryZanInfo(postInfo, userObj, "").size();
        	
        } 
        
        int zanNum = zanInfoService.queryZanInfo(postInfo, null, "").size();
	...
	public void update(@Validated PostInfo postInfo, BindingResult br,
		...
		String postPhotoFileName = this.handlePhotoUpload(request, "postPhotoFile");
		if(!postPhotoFileName.equals("upload/NoImage.jpg"))postInfo.setPostPhoto(postPhotoFileName); 
	...
	public String delete(@PathVariable Integer postInfoId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  postInfoService.deletePostInfo(postInfoId);
	            request.setAttribute("message", "博文删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "博文删除失败!");
				return "error";
	...
	public void OutToExcel(String title,@ModelAttribute("userObj") UserInfo userObj,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "博文id","博文标题","博文图片","浏览量","发布人","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<postInfoList.size();i++) {
        	PostInfo postInfo = postInfoList.get(i); 
        	dataset.add(new String[]{postInfo.getPostInfoId() + "",postInfo.getTitle(),postInfo.getPostPhoto(),postInfo.getHitNum() + "",postInfo.getUserObj().getName(),postInfo.getAddTime()});
        }
        /*
