...
public class UserInfoService {
    ...
    /*按照查询条件分页查询用户记录*/
    public ArrayList<UserInfo> queryUserInfo(String user_name,String name,String birthDate,String telephone,String shzt,int currentPage) throws Exception { 
    	...
    	if(!shzt.equals("")) where = where + " and t_userInfo.shzt like '%" + shzt + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return userInfoMapper.queryUserInfo(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<UserInfo> queryUserInfo(String user_name,String name,String birthDate,String telephone,String shzt) throws Exception  { 
     	String where = "where 1=1";
    	if(!user_name.equals("")) where = where + " and t_userInfo.user_name like '%" + user_name + "%'";
    	if(!name.equals("")) where = where + " and t_userInfo.name like '%" + name + "%'";
    	if(!birthDate.equals("")) where = where + " and t_userInfo.birthDate like '%" + birthDate + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String user_name,String name,String birthDate,String telephone,String shzt) throws Exception {
     	String where = "where 1=1";
    	if(!user_name.equals("")) where = where + " and t_userInfo.user_name like '%" + user_name + "%'";
    	if(!name.equals("")) where = where + " and t_userInfo.name like '%" + name + "%'";
    	if(!birthDate.equals("")) where = where + " and t_userInfo.birthDate like '%" + birthDate + "%'";
	...
	public boolean checkLogin(String userName, String password) throws Exception { 
		...
		if(db_user == null) { 
			...
			System.out.print(this.errMessage);
			return false;
		} else if( !db_user.getShzt().equals("已审核")) {
			this.errMessage = " 管理员未审核不能登录! ";
			System.out.print(this.errMessage);
			return false;
		}
