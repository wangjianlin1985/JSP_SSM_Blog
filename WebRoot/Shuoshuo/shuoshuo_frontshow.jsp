...
<div class="container">
	<ul class="breadcrumb">
  		...
  		<li><a href="<%=basePath %>Shuoshuo/frontlist">日志信息</a></li>
	...
	<div class="row bottom15"> 
		...
		<div class="col-md-10 col-xs-6"><a href="<%=basePath %>UserInfo/<%=shuoshuo.getUserObj().getUser_name() %>/frontshow" target="_blank"><%=shuoshuo.getUserObj().getName() %></a></div>
	</div>
	<div class="row bottom15"> 
	...
	<div class="row bottom15">
		...
		<div class="col-md-6 col-xs-6">
			...
			&nbsp;&nbsp;共&nbsp;<a href="<%=basePath %>ShuoshuoZan/frontlist?shuoshuoObj.shuoshuoId=<%=shuoshuo.getShuoshuoId() %>" style="color:red;text-decoration: underline;"><%=zanNum %>人</a>&nbsp;点赞
		</div>
	</div>
	<div class="row bottom15"> 
		...
		<div class="col-md-8 col-xs-7">
			...
			<div class="row" style="margin-top: 20px;">
				...
				<div class="col-md-7 col-xs-5"><%=comment.getCommentContent() %></div>
...
function userReply() {
	...
	$.ajax({
		...
		dataType: "json",
		data: {
			"comment.shuoshuoObj.shuoshuoId": <%=shuoshuo.getShuoshuoId() %>,
...
function userZan() {
	$.ajax({
		url : basePath + "ShuoshuoZan/userZan",
		type : "post",
		dataType: "json",
		data: {
			"shuoshuoZan.shuoshuoObj.shuoshuoId": <%=shuoshuo.getShuoshuoId() %>,
		},
		success : function (data, response, status) {
			//var obj = jQuery.parseJSON(data);
			if(data.success){
